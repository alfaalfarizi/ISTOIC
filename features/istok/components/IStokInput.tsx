...
export const IStokInput = memo(({ 
 ...
 codex/refactor-background-colors-in-components
        <div className="bg-[var(--bg-card)]/90 backdrop-blur-md border-t border-[var(--border-base)] p-3 z-20 pb-[max(env(safe-area-inset-bottom),1rem)] pt-3 shrink-0 shadow-2xl">
 ...
 main
                ...
                <div className="flex items-center gap-2 mb-3 animate-slide-up px-1 overflow-x-auto no-scrollbar">
                    ...
                    {translateLang && (
                        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-purple-500/20 border border-purple-500/30 text-purple-400 text-[10px] font-black uppercase tracking-wider shrink-0">
                            ...
                            <button onClick={()=>setTranslateLang(null)} className="ml-1 hover:text-[var(--text-main)]"><X size={10}/></button>
                        </div>
                    )}
                    ...
                    {showAiMenu && (
                        <>
                            <button onClick={()=>handleAiAction('REPLY')} className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 text-[var(--text-main)] text-[10px] font-bold shrink-0 transition-colors">
                                <Sparkles size={12} className="text-yellow-400"/> SMART REPLY
                            </button>
                            <button onClick={()=>handleAiAction('REFINE')} disabled={!text} className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 text-[var(--text-main)] text-[10px] font-bold shrink-0 transition-colors disabled:opacity-50">
            ...
            <div className="flex gap-2 items-end">
                {/* Tools Toggle */}
                <button onClick={onSendFile} className="p-3 bg-white/5 rounded-full text-[var(--text-muted)] hover:text-[var(--text-main)] transition-colors border border-transparent hover:border-[var(--border-base)] active:scale-95"><Zap size={20}/></button>
                
                {/* Input Container */}
                <div className={`flex-1 bg-white/5 rounded-[24px] px-2 border border-[var(--border-base)] focus-within:border-emerald-500/30 transition-all relative flex items-center ${isAiThinking ? 'animate-pulse border-purple-500/50' : ''}`}>
                    
                    {/* Magic Wand (AI Trigger) */}
                    <button 
                        onClick={() => setShowAiMenu(!showAiMenu)}
                        className={`p-2 rounded-full transition-colors ${showAiMenu ? 'text-purple-400 bg-purple-500/10' : 'text-[var(--text-muted)] hover:text-purple-400'}`}
                    >
                        <Sparkles size={16} />
                    </button>
                    ...
                    <input 
                        ...
                        className="flex-1 bg-transparent outline-none text-[var(--text-main)] text-sm placeholder:text-[var(--text-muted)] px-2 py-3" 
                    ...
                    <button 
                        ...
                        className={`p-2 rounded-full transition-colors ${translateLang ? 'text-cyan-400 bg-cyan-500/10' : 'text-[var(--text-muted)] hover:text-cyan-400'}`}
                ...
                {text.trim() ? (
                    ...
                    <button 
                        ...
                        className={`p-3 rounded-full transition-all ${isRecording ? 'bg-red-500 text-white shadow-[0_0_15px_red] animate-pulse' : 'bg-white/5 text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}
                    >
                        <Mic size={20}/>
                    </button>
                )}
            ...
            <div className="flex justify-center mt-2 opacity-50 hover:opacity-100 transition-opacity">
                <button onClick={onToggleTtl} className={`flex items-center gap-1 px-3 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest transition-all ${ttlMode > 0 ? 'bg-red-500/10 text-red-500 border border-red-500/20' : 'text-[var(--text-muted)] hover:text-[var(--text-muted)]'}`}>
