...
export const AudioMessagePlayer = React.memo(({ src, duration = 0, isMasked, mimeType = 'audio/webm' }: AudioMessagePlayerProps) => {
 ...
 codex/refactor-background-colors-in-components
              'bg-[var(--bg-surface)] border border-[var(--border-base)] hover:border-emerald-500/30'}
                ...
                <div className="flex items-center justify-between w-full px-1">
                     <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-mono font-bold text-[var(--text-muted)]">
                            {formatTime(currentTime)} <span className="text-[var(--text-muted)]">/</span> {formatTime(duration)}
                        </span>
                    </div>
