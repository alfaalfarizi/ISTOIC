...
export const ParticipantList: React.FC<ParticipantListProps> = ({ participants, isOpen, onClose }) => {
 ...
 main
        ...
        `}>
            <div className="p-4 border-b border-[var(--border-base)] flex items-center justify-between bg-white/[0.02]">
                <div className="flex items-center gap-2 text-[var(--text-main)]">
                    <Users size={16} className="text-emerald-500" />
                    <span className="text-[10px] font-black uppercase tracking-widest">ANGGOTA ROOM ({participants.length})</span>
                </div>
                <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-main)] text-xs font-bold">TUTUP</button>
            </div>
                    ...
                    <div key={p.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-white/5 group">
                        <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="text-[9px] font-mono text-[var(--text-muted)] truncate">{p.id.slice(0,8)}...</p>
                            </div>
                        </div>
