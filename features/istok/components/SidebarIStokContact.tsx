            ...
            `}>
                ...
                <div className="pt-safe px-6 pb-6 border-b border-[var(--border-base)] bg-[var(--bg-main)] relative overflow-hidden shrink-0">
                    ...
                    <div className="flex justify-between items-start mb-6 relative z-10 pt-4">
                        <h2 className="text-xs font-black uppercase tracking-[0.2em] text-[var(--text-main)] flex items-center gap-2">
                            <Zap size={14} className="text-emerald-500 fill-current"/> ISTOK_SECURE
                        </h2>
                        <button onClick={onClose} className="p-1 hover:bg-white/10 rounded-full transition"><X size={18} className="text-[var(--text-muted)] hover:text-[var(--text-main)]" /></button>
                    </div>

                    <div className="flex items-center gap-4 mb-4 relative z-10">
                        {profile.photoURL ? (
                            <img src={profile.photoURL} alt="Profile" className="w-12 h-12 rounded-full border border-[var(--border-base)] shadow-lg object-cover"/>
                        ) : (
                            <div className="w-12 h-12 rounded-full bg-emerald-600 flex items-center justify-center text-[var(--text-main)] font-bold text-lg shadow-lg border border-[var(--border-base)]">
                                {profile.username.substring(0,1)}
                            </div>
                        )}
                        <div className="flex-1 overflow-hidden">
                            <h3 className="text-[var(--text-main)] font-bold text-sm truncate">{profile.username}</h3>
                            <p className="text-[var(--text-muted)] text-[10px] truncate">{profile.email || "No Email Linked"}</p>
                        </div>
                    </div>

                    <div className="p-3 bg-white/5 rounded-xl border border-[var(--border-base)] backdrop-blur-md relative group">
                        <div className="flex items-center justify-between mb-1">
                            <p className="text-[9px] font-bold text-[var(--text-muted)] uppercase tracking-wider">YOUR ISTOIC ID</p>
                            <button onClick={handleCopyId} className="text-[var(--text-muted)] hover:text-emerald-400 transition">
                        ...
                        <div className="absolute -bottom-1 -right-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={onLogout} className="p-1.5 bg-red-900/50 rounded-tl-lg text-red-200 hover:text-[var(--text-main)]" title="Sign Out"><LogOut size={10}/></button>
                        </div>
                    </div>
                </div>
                ...
                <div className="px-4 py-3 space-y-3 bg-[var(--bg-card)] shrink-0">
                    <div className="flex bg-white/5 p-1 rounded-xl border border-[var(--border-base)]">
                        ...
                        <button 
                            ...
                            className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${activeTab === 'CONTACTS' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/20' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}
                        >
                    ...
                    <div className="relative group">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)] group-focus-within:text-emerald-500 transition-colors" size={14} />
                        <input 
                            type="text" 
                            value={search} 
                            onChange={(e)=>setSearch(e.target.value)} 
                            placeholder={activeTab === 'CHATS' ? "Cari riwayat..." : "Cari teman..."}
                            className="w-full bg-white/5 border border-[var(--border-base)] rounded-xl py-2.5 pl-9 pr-3 text-xs text-[var(--text-main)] placeholder:text-[var(--text-muted)] outline-none focus:border-emerald-500/50 transition-all font-mono"
                        />
                    </div>
                </div>
                    ...
                    {activeTab === 'CHATS' && (
                        <>
                            {filteredSessions.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-40 opacity-30 text-[var(--text-muted)] space-y-2">
                                        ...
                                        <div 
                                            ...
                                            className={`
                                                ...
                                                ${isConnected 
                                                    ...
                                                    : 'bg-white/5 border-[var(--border-base)] hover:bg-white/10 hover:border-[var(--border-base)]'}
                                            `}
                                        >
                                            <div className="relative">
                                                {s.photoURL ? (
                                                     <img src={s.photoURL} alt="User" className="w-10 h-10 rounded-full object-cover shadow-sm border border-[var(--border-base)]"/>
                                                ) : (
                                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-[var(--text-main)] font-bold text-xs shadow-inner ${s.isContact ? 'bg-indigo-600' : 'bg-neutral-700'}`}>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-center mb-0.5">
                                                    <h4 className={`text-xs font-bold truncate ${isConnected ? 'text-emerald-400' : 'text-[var(--text-main)]'}`}>
                                                        {s.displayName}
                                                    </h4>
                                                    <span className="text-[8px] font-mono text-[var(--text-muted)]">{formatRelativeTime(s.lastSeen)}</span>
                                                </div>
                        ...
                        <>
                            {!isAddingContact ? (
                                <button 
                                    ...
                                    className="w-full py-3 border border-dashed border-[var(--border-base)] bg-white/5 rounded-xl text-[var(--text-muted)] hover:text-emerald-400 hover:border-emerald-500/30 hover:bg-emerald-500/5 transition-all text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 mb-4 active:scale-95"
                                >
                                    <UserPlus size={14}/> TAMBAH TEMAN
                                </button>
                            ) : (
                                <div className="p-4 bg-white/5 rounded-2xl border border-[var(--border-base)] mb-4 animate-slide-up">
                                    <h4 className="text-[10px] font-black text-[var(--text-main)] uppercase tracking-widest mb-3 flex items-center gap-2"><UserPlus size={12}/> KONEKSI BARU</h4>
                                    
                                    <div className="space-y-3">
                                        <div className="space-y-1">
                                            <label className="text-[8px] text-[var(--text-muted)] font-bold ml-1">NAMA ALIAS</label>
                                            <input 
                                                value={contactName} 
                                                onChange={e=>setContactName(e.target.value)} 
                                                className="w-full bg-[var(--bg-surface)] border border-[var(--border-base)] p-2.5 rounded-lg text-xs text-[var(--text-main)] focus:border-emerald-500 outline-none"
                                                placeholder="Contoh: Bos Besar"
                                            />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[8px] text-[var(--text-muted)] font-bold ml-1">ISTOIC ID (e.g. BARISTA)</label>
                                            <div className="flex items-center bg-[var(--bg-surface)] border border-[var(--border-base)] rounded-lg focus-within:border-emerald-500 transition-all px-2.5">
                                                <span className="text-[10px] font-black text-emerald-500 mr-1">ISTOIC-</span>
                                                <input 
                                                    value={targetId} 
                                                    onChange={e=>setTargetId(e.target.value.toUpperCase())} 
                                                    className="w-full bg-transparent border-none p-2 text-xs text-[var(--text-main)] outline-none font-mono uppercase"
                                                    placeholder="KODENAME"
                                                />
                                            </div>
                                        </div>
                                        <div className="flex gap-2 pt-2">
                                            <button onClick={()=>setIsAddingContact(false)} className="flex-1 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-[10px] font-bold text-[var(--text-muted)]">BATAL</button>
                            ...
                            {filteredContacts.length === 0 && !isAddingContact ? (
                                ...
                                filteredContacts.map(c => (
                                    <div 
                                        key={c.id} 
                                        className="group p-3 rounded-xl bg-white/[0.02] border border-[var(--border-base)] hover:bg-white/5 transition-all flex items-center gap-3 cursor-pointer active:scale-98"
                                    >
                                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-900 to-indigo-700 text-[var(--text-main)] flex items-center justify-center font-bold text-sm shadow-lg border border-[var(--border-base)]">
                                        ...
                                        <div className="flex-1 overflow-hidden" onClick={() => onCallContact(c)}>
                                            <h4 className="text-xs font-bold text-[var(--text-main)] truncate">{c.name}</h4>
                                            <p className="text-[9px] text-[var(--text-muted)] truncate font-mono">{c.id}</p>
                                        </div>
                                        <div className="flex gap-1">
                                            ...
                                            <button 
                                                ...
                                                className="p-2 bg-white/5 text-[var(--text-muted)] rounded-lg hover:bg-red-500 hover:text-white transition-colors"
                                                title="Hapus Kontak"
                                            >
                                                <Trash2 size={14}/>
                                            </button>
